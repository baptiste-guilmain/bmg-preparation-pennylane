...
const [html, css, profiles, app] = await Promise.all([
  source('index.html'), source('styles.css'), source('profiles.js'), source('app.js')
]);
...
const workspaceApp = app
  ...
  .replace(/[\s\S]*?\/\/ Envoi direct à Pennylane \(bêta\)[\s\S]*?async function makeXlsx\(/, 'function resetPennylanePanel(){}\nasync function makeXlsx(');
